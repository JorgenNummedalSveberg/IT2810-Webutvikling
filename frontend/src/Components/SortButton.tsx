...
function SortButton(props: {sort: string, refresh: any}) {
    const dispatch = useDispatch();
    const active = useSelector((state: state) => state.filter.sort) === props.sort;

    const [asc, setAsc] = useState(active)
    ...
    return (
        <div className={"SortButton"} onClick={toggleSort}>
            <svg style={{"transform":"rotate("+(active ? (asc ? 90 : (-90)) : 0)+"deg)"}} width="258" height="452" viewBox="0 0 258 452" fill="none" xmlns="http://www.w3.org/2000/svg">
